...
class InternalStructuresInterface {
    ...
    static noncedGeographicalPointFromNumber(latitude, longitude) {
        const geographicalPoint = InternalStructuresInterface.geographicalPointFromNumber(latitude, longitude);
        const nonce = Field(Math.floor(Math.random() * 1000000));
    ...
    static geographicalPointFromNumber(latitude, longitude) {
        ...
        const point = new GeographicalPoint({
...
describe('CoordinatesInPolygon', () => {
    ...
    describe('OR/AND combinaton', () => {
        ...
        beforeAll(async () => {
            ...
            andProoForInsideOutsideC1 = await CoordinatesInPolygon.AND(proofInsideCoordinate1InInsidePolygon1, proofInsideCoordinate1NotInOutsidePolygon1);
            andProofForInsideInsideC1 = await CoordinatesInPolygon.AND(proofInsideCoordinate1InInsidePolygon1, proofInsideCoordinate1InInsidePolygon2);
        });
        ...
        describe('when OR inside/outside proof is ANDed with a proof outside polygon', () => {
            it('returns proof outside polygon', async () => {
                const andInsideOutsideProofAndOutsideProof = await CoordinatesInPolygon.AND(orProofForInsideOutsideC1, proofInsideCoordinate1NotInOutsidePolygon1);
        ...
        describe('when AND inside/outside proof is ANDed with a proof outside polygon', () => {
            it('returns proof outside polygon', async () => {
                const andInsideOutsideProofAndOutsideProof = await CoordinatesInPolygon.AND(andProoForInsideOutsideC1, proofInsideCoordinate1NotInOutsidePolygon1);
                const publicOutput = andInsideOutsideProofAndOutsideProof.publicOutput;
                const isInPolygonObtained = Bool(publicOutput.isInPolygon);
                expect(isInPolygonObtained).toEqual(Bool(false));
            });
        ...
        describe('when OR inside/outside proof is ANDed with a proof inside polygon', () => {
            it('returns proof inside polygon', async () => {
                const andInsideOutsideProofAndOutsideProof = await CoordinatesInPolygon.AND(orProofForInsideOutsideC1, proofInsideCoordinate1InInsidePolygon1);
                const publicOutput = andInsideOutsideProofAndOutsideProof.publicOutput;
                const isInPolygonObtained = Bool(publicOutput.isInPolygon);
                expect(isInPolygonObtained).toEqual(Bool(true));
            });
        ...
        describe('when OR inside/outside proof is ORed with a proof inside polygon', () => {
            it('returns proof inside polygon', async () => {
                const orInsideOutsideProofAndOutsideProof = await CoordinatesInPolygon.OR(orProofForInsideOutsideC1, proofInsideCoordinate1InInsidePolygon1);
        ...
        describe('when AND inside/outside proof is ORed with OR inside/outside polygon', () => {
            it('returns proof inside polygon', async () => {
                const orInsideOutsideProofAndOutsideProof = await CoordinatesInPolygon.OR(andProoForInsideOutsideC1, orProofForInsideOutsideC1);
                const publicOutput = orInsideOutsideProofAndOutsideProof.publicOutput;
                const isInPolygonObtained = Bool(publicOutput.isInPolygon);
                expect(isInPolygonObtained).toEqual(Bool(true));
            });
        ...
        describe('when AND inside/outside proof is ORed with a OR outside/outside polygon', () => {
            it('returns proof outside polygon', async () => {
                const orInsideOutsideProofAndOutsideProof = await CoordinatesInPolygon.OR(andProoForInsideOutsideC1, orProofForOutsideOutsideC1);
                const publicOutput = orInsideOutsideProofAndOutsideProof.publicOutput;
                const isInPolygonObtained = Bool(publicOutput.isInPolygon);
                expect(isInPolygonObtained).toEqual(Bool(false));
            });
        ...
        describe('when OR inside/outside proof is ORed with a proof outside polygon', () => {
            it('returns proof inside polygon', async () => {
                const orInsideOutsideProofAndOutsideProof = await CoordinatesInPolygon.OR(orProofForInsideOutsideC1, proofInsideCoordinate1NotInOutsidePolygon1);
                const publicOutput = orInsideOutsideProofAndOutsideProof.publicOutput;
                const isInPolygonObtained = Bool(publicOutput.isInPolygon);
                expect(isInPolygonObtained).toEqual(Bool(true));
            });
