<script lang="ts">
	...
	import type { ZKExactGeolocationMetadataCircuitProof } from 'zklocus';
	import type { ZKGeoPoint } from 'zklocus';
	let L;
	...
	export let zkProof: ZKExactGeolocationMetadataCircuitProof;
	...
	function clearPolygonLayers() {
		...
		polygonLayers = [];
		map.setView([0, 0], 100); // Reset map view to default
	}
	...
	function displayPolygonsFromZKProof(proof: ZKExactGeolocationMetadataCircuitProof) {
		clearPolygonLayers();

		let zkGeoPoint: ZKGeoPoint = zkProof.zkGeoPoint;

		marker = L.marker([zkGeoPoint.latitude.normalized, zkGeoPoint.longitude.normalized], { draggable: false}).addTo(map);
		map.setView([zkGeoPoint.latitude.normalized, zkGeoPoint.longitude.normalized], 13); // Default to a world view
	}
	...
	async function verifyProof() {
		try {
			...
			const zkGeoPoint = zkProof.zkGeoPoint;
			if (isOk) {

				L.popup()
					.setLatLng([zkGeoPoint.latitude.normalized, zkGeoPoint.longitude.normalized])
					.setContent(
						`Exact geolocation verified successfully. You have just proved your exact geolocation with metadata to any third party. Metadata: ${zkProof.metadata}`
					)
					.openOn(map);
